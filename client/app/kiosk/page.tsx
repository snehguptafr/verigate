...
export default function KioskPage() {
  ...
  const [hostVisitCounts, setHostVisitCounts] = useState<Record<string, number>>({})

  const [form, setForm] = useState({
    name: "", email: "", phone: "",
    tenantId: "", hostId: "", purpose: ""
  })
  ...
  useEffect(() => {
    if (!form.tenantId) { setHosts([]); return }
  ...
  const handlePhoneLookup = async () => {
    ...
    const res = await fetch(`${getAPI()}/visitors/by-phone?phone=${form.phone}`)
  ...
  const handleReturningSubmit = async () => {
    ...
    await submitVisit(matchedVisitor.id, matchedVisitor.tenantId || form.tenantId)
  }
  ...
  const handleNewVisitorSubmit = async () => {
    ...
    else if (!/^\d{10}$/.test(form.phone)) newErrors.phone = "10 digits required"
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50">
      <div className="bg-white p-8 rounded-2xl shadow w-full max-w-md">
        ...
        <div className="flex flex-col gap-4">
            ...
            >
              ...
              {sortedHosts.map(h => (
                <option key={h.id} value={h.id}>
                  {h.name}{hostVisitCounts[h.id] ? ` (${hostVisitCounts[h.id]} visit${hostVisitCounts[h.id] > 1 ? "s" : ""})` : ""}
                </option>
              ))}
            </select>
            {errors.hostId && (
          ...
          <button onClick={reset} className="text-sm text-gray-400 underline text-center">
            Not you?
          </button>
        </div>
      </div>
  ...
  if (step === "new-form") return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 py-8">
      <div className="bg-white p-8 rounded-2xl shadow w-full max-w-md">
        ...
        <p className="text-gray-500 text-sm mb-6">Fill in your details to check in</p>
